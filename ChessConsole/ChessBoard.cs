...
namespace ChessConsole
{
    ...
    public class ChessBoard
    {
        ...
        public ChessBoard(bool classic = true)
        {
            Reset(classic);
        }
        ...
        /// </summary>
        public void Reset(bool classic = true)
        {
            cells = new Cell[8, 8];
            for (int i = 0; i < 8; i++)
            {
        ...
        public void Chess960()
        {
            ...
            int firstBishopPos = rnd.Next(0, 7);
            if (firstBishopPos % 2 == 0) firstBishopPos++;
            int secondBishopPos = rnd.Next(0, 7);
            if (secondBishopPos % 2 == 1) secondBishopPos++;
            if (firstBishopPos == 9) firstBishopPos = 7;

            addPiece(cells[firstBishopPos, 7], new Bishop(PlayerColor.Black));
            addPiece(cells[secondBishopPos, 7], new Bishop(PlayerColor.Black));

            addPiece(cells[firstBishopPos, 0], new Bishop(PlayerColor.White));
            addPiece(cells[secondBishopPos, 0], new Bishop(PlayerColor.White));
            taken[firstBishopPos] = true;
            taken[secondBishopPos] = true;

            int tempPos = FindRandEmptySpot(taken, rnd);
            addPiece(cells[tempPos, 7], new Knight(PlayerColor.Black));
            addPiece(cells[tempPos, 0], new Knight(PlayerColor.White));
            taken[tempPos] = true;
            tempPos = FindRandEmptySpot(taken, rnd);
            addPiece(cells[tempPos, 7], new Knight(PlayerColor.Black));
            addPiece(cells[tempPos, 0], new Knight(PlayerColor.White));
            taken[tempPos] = true;
            tempPos = FindRandEmptySpot(taken, rnd);
            addPiece(cells[tempPos, 7], new Queen(PlayerColor.Black));
            addPiece(cells[tempPos, 0], new Queen(PlayerColor.White));
            taken[tempPos] = true;

            
            tempPos = FindFirstEmptySpot(taken, rnd);
            addPiece(cells[tempPos, 7], new Rook(PlayerColor.Black));
            addPiece(cells[tempPos, 0], new Rook(PlayerColor.White));
            taken[tempPos] = true;
            tempPos = FindFirstEmptySpot(taken, rnd);
            addPiece(cells[tempPos, 7], (blackKing = new King(PlayerColor.Black)));
            addPiece(cells[tempPos, 0], (whiteKing = new King(PlayerColor.White)));
            taken[tempPos] = true;
            tempPos = FindFirstEmptySpot(taken, rnd);
            addPiece(cells[tempPos, 7], new Rook(PlayerColor.Black));
            addPiece(cells[tempPos, 0], new Rook(PlayerColor.White));
        ...
        private int FindRandEmptySpot(bool[] taken, Random rnd)
        {
            bool found = false;
            int output = 0;
            while (!found)
        ...
        private int FindFirstEmptySpot(bool[] taken, Random rnd)
        {
            bool found = false;
            int output = 0;
            while (!found) 
            {
                if (!taken[output]) found = true;
                else output++;
            }
            return output;
        }
